...
IF EXISTS (SELECT 1 FROM sys.default_constraints dc JOIN sys.columns c
  ...
  ALTER TABLE bd_manu_order DROP CONSTRAINT [DF__bd_manu_or__单据状态__24766F2A];
GO
...
IF OBJECT_ID('v_manu_order_detail','V') IS NULL EXEC('CREATE VIEW v_manu_order_detail AS SELECT h.*, l.[产品编码], l.[存货图片], l.[产品名称], l.[规格型号], l.[型号], l.[生产单位], l.[数量], l.[齐套数量(主)], l.[累计汇报套数(工序单位)], l.[可用量], l.[现存量], l.[图号], l.[单重], l.[总重], l.[需求令号] FROM bd_manu_order h LEFT JOIN bl_manu_order l ON h.[合同号]=l.[合同号]');
GO
