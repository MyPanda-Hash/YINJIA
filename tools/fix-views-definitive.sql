-- definitive:一次性重建全部 20 个视图(10 DETAIL 自动发现列 + 10 STATS 固定列),带 asp_cancel
USE HSDZ_MES;
SET NOCOUNT ON;
-- 1. 全量 DROP
...
DECLARE @code varchar(50), @h sysname, @l sysname, @nc sysname, @lc nvarchar(max), @hc nvarchar(max), @sql nvarchar(max);
...
WHILE @@FETCH_STATUS = 0 BEGIN
  ...
  WHERE c.object_id = OBJECT_ID(@l) AND c.name NOT IN ('id','asp_user1','asp_time1','asp_cancel','asp_user2','asp_time2', @nc)
    AND c.name NOT IN (SELECT c2.name FROM sys.columns c2 WHERE c2.object_id = OBJECT_ID(@h))
  ORDER BY c.column_id;
  -- 头表列显式列出(排除 asp_cancel:外层包装会补 NULL asp_cancel,头表自带则视图列名重复)
  SET @hc = '';
  SELECT @hc = @hc + CASE WHEN @hc = N'' THEN N'' ELSE N',' END + N'h.[' + c.name + N']' FROM sys.columns c
  WHERE c.object_id = OBJECT_ID(@h) AND c.name <> 'asp_cancel'
  ORDER BY c.column_id;
  SET @sql = N'CREATE VIEW v_' + @code + '_detail AS SELECT t.*, CAST(NULL AS char(1)) AS asp_cancel FROM (SELECT ' + @hc + @lc + N' FROM ' + @h + N' h LEFT JOIN ' + @l + N' l ON h.[' + @nc + N']=l.[' + @nc + N']) t';
  EXEC(@sql);
  FETCH NEXT FROM p INTO @code, @h, @l, @nc;
END
CLOSE p; DEALLOCATE p;
PRINT 'DETAIL 10 OK';
-- 3. STATS(固定列,asp_cancel 在 GROUP BY)
...
EXEC('CREATE VIEW v_finish_in_stats AS SELECT h.[单据日期], h.asp_cancel, l.[存货名称], l.[规格型号], l.[计量单位], COUNT(DISTINCT h.[单据编号]) AS [单据数], SUM(COALESCE(l.[实收数量],0)) AS [实收数量], SUM(COALESCE(l.[金额],0)) AS [金额] FROM bd_finish_in h LEFT JOIN bl_finish_in l ON h.[单据编号]=l.[单据编号] GROUP BY h.[单据日期], h.asp_cancel, l.[存货名称], l.[规格型号], l.[计量单位]');
EXEC('CREATE VIEW v_other_in_stats AS SELECT h.[单据日期], h.asp_cancel, l.[存货名称], l.[规格型号], l.[计量单位], COUNT(DISTINCT h.[单据编号]) AS [单据数], SUM(COALESCE(l.[实收数量],0)) AS [实收数量], SUM(COALESCE(l.[金额],0)) AS [金额] FROM bd_other_in h LEFT JOIN bl_other_in l ON h.[单据编号]=l.[单据编号] GROUP BY h.[单据日期], h.asp_cancel, l.[存货名称], l.[规格型号], l.[计量单位]');
EXEC('CREATE VIEW v_sale_out_stats AS SELECT h.[单据日期], h.asp_cancel, l.[存货名称], l.[规格型号], l.[计量单位], COUNT(DISTINCT h.[单据编号]) AS [单据数], SUM(COALESCE(l.[数量],0)) AS [数量], SUM(COALESCE(l.[金额],0)) AS [金额] FROM bd_sale_out h LEFT JOIN bl_sale_out l ON h.[单据编号]=l.[单据编号] GROUP BY h.[单据日期], h.asp_cancel, l.[存货名称], l.[规格型号], l.[计量单位]');
...
EXEC('CREATE VIEW v_manu_order_stats AS SELECT h.asp_cancel, l.[产品名称], COUNT(DISTINCT h.[合同号]) AS [单据数], SUM(COALESCE(l.[计划数量],0)) AS [计划数量], SUM(COALESCE(l.[完工数量],0)) AS [完工数量] FROM bd_manu_order h LEFT JOIN bl_manu_order l ON h.[合同号]=l.[合同号] GROUP BY h.asp_cancel, l.[产品名称]');
