...
IF OBJECT_ID('yj_role') IS NULL CREATE TABLE yj_role (
    id        int IDENTITY(1,1) PRIMARY KEY,
...
IF OBJECT_ID('yj_role_panel') IS NULL CREATE TABLE yj_role_panel (
    id          int IDENTITY(1,1) PRIMARY KEY,
...
GO
-- ===== 公司组织架构种子(仅 yj_dept 为空时初始化) =====
IF NOT EXISTS (SELECT 1 FROM yj_dept)
BEGIN
...
END
GO
-- 角色:只补缺失的 role_code,不动已有行(角色 id 被用户数据引用,必须稳定)
IF NOT EXISTS (SELECT 1 FROM yj_role WHERE role_code = 'admin')
    INSERT INTO yj_role (role_code, role_name, remark, is_admin)
    VALUES ('admin', N'管理员', N'全部面板与审批权限', 'Y');
IF NOT EXISTS (SELECT 1 FROM yj_role WHERE role_code = 'user')
    INSERT INTO yj_role (role_code, role_name, remark, is_admin)
    VALUES ('user', N'普通用户', N'按角色授权面板', 'N');
GO
-- yj_user 扩展:部门/角色/启用
...
GO
IF USER_NAME() <> 'yinjia' GRANT SELECT, INSERT, UPDATE, DELETE ON yj_dept TO yinjia;
IF USER_NAME() <> 'yinjia' GRANT SELECT, INSERT, UPDATE, DELETE ON yj_role TO yinjia;
IF USER_NAME() <> 'yinjia' GRANT SELECT, INSERT, UPDATE, DELETE ON yj_role_panel TO yinjia;
GO
