...
const SPEC = {
  ...
  BD_DEPT: { panel: 'DEPT', map: { number: ['部门编码'], name: ['部门名称'], parent_name: ['上级部门'], parent_number: ['上级编码'], level: ['级次'], long_number: ['长编码'], full_name: ['部门全称'], comment: ['备注'], enable: ['停用'] } },
  ...
  BD_STORE: { panel: 'WH', map: { number: ['仓库编码'], name: ['仓库名称'], address: ['仓库地址'], storekeeper_name: ['负责人'], storekeeper_number: ['仓库管理员编码'], country_name: ['国家'], province_name: ['省'], city_name: ['市'], district_name: ['区'], allow_negative: ['允许零库存出库'], is_allow_freight: ['启用仓位管理'], enable: ['停用'], SKIP_mobile: ['联系电话'] } },
  ...
  BD_SUPPLIER: { panel: 'GFDA', map: { number: ['dm'], name: ['mc'], group_name: ['gysfl'], saler_name: ['ywman'], sale_dept_name: ['采购员部门'], taxpayer_no: ['sui_no'], deduct: ['自动抵扣预收款'], remark: ['bz'], enable: ['停用(asp_cancel)'], 'account_entity.income_bank_name': ['bank'], 'SKIP_account_entity.income_acc_no': ['bank_no'], SKIP_tel: ['tel'] } },
};
...
for (const [code, spec] of Object.entries(SPEC)) {
  ...
  const apiKeys = new Set(rec ? (rec.detailKeys && rec.detailKeys.length ? rec.detailKeys : rec.listKeys) : []);
  ...
  // mapArchive 实际输出
  const full = rec && (rec.detailFull || rec.listFull) || {};
