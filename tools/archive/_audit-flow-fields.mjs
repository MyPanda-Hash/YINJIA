...
const FLOW_HEAD_SYNONYMS = {
  ...
  'PU_ORDER|QC_RECV': [['供应商编码', '供应商代码'], ['单据编号', '采购订单号']],
  'QC_RECV|QC_INSP': [['采购订单号', '采购订单号'], ['批次号', '批次号'], ['批次键', '批次键'], ['单号', '暂收单号']],
  // 2026-09-22:采购订单→采购入库单的免检直达已取消,改由送料暂收单去向分流 —— 本跳新增
  'QC_RECV|PURCHASE_IN': [['供应商代码', '供应商编码']],
  'QC_INSP|PURCHASE_IN': [['单号', '外部单据号'], ['采购订单号', '采购订单号'], ['批次号', '批次号'], ['批次键', '批次键'], ['供应商代码', '供应商编码']],
...
const CHAIN = [
  ...
  // 2026-09-22:所有采购订单都必须先到暂收;暂收再分流「走检验」或「免检直达」
  { from: 'QC_RECV', to: 'QC_INSP', name: '来料暂收单 → 来料检验单' },
  { from: 'QC_RECV', to: 'PURCHASE_IN', name: '来料暂收单 → 采购入库单(免检直达)' },
