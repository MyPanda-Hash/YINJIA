...
#
# 2026-09-22: package folder is now a parameter (was hard-coded to pkg-incr-20260921).
...
# 0) start from a CLEAN tools\ (2026-09-22): an older assembly left 79 extra .sql
#    files behind and the "scripts in package but not in manifest" gate caught it.
#    Nothing under pkg\tools is hand-edited, so wiping it is safe; the gate below
#    re-proves the package content afterwards.
...
# Read the manifest as UTF-8 EXPLICITLY: PowerShell 5.1's Get-Content decodes a
# BOM-less UTF-8 file with the OEM codepage and SWALLOWS line breaks, which made
# this parse 223 entries as 147 and fail the exact-match gate (repo pitfall C6,
# hit again here on 2026-09-22).
...
#    check-migrations.sql / .bat are not generated by this script: they are carried
#    over from the previous package. deploy-incremental.bat step 5 runs the .sql by
#    ABSOLUTE package-root path and requires its "RESULT: SCHEMA-COMPLETE" line, so a
#    package missing it fails that gate (2026-09-22: the 09-22 folder had lost it).
$carry = @('check-migrations.bat', 'check-migrations.sql', 'steps.md')
foreach ($c in $carry) {
  $dstC = Join-Path $pkg $c
  if (Test-Path $dstC) { continue }
