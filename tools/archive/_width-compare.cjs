/**
 * _width-compare.cjs — 对比 RD_FILTER_EFF 与新面板的纸张宽度(一致性检查)
 */
...
async function main() {
  ...
  try {
    ...
    for (const pc of ['RD_FILTER_EFF', 'RD_ALKALINE']) {
      await navigate(`${FRONT}/#/panelx/list/${pc}`)
      await sleep(3000)
      const r = await evaluate(`(() => {
  const sheet = document.querySelector('.record-sheet')
  const layout = document.querySelector('.approval-layout')
  return {
    sheetW: sheet ? sheet.getBoundingClientRect().width : null,
    layoutW: layout ? layout.getBoundingClientRect().width : null,
    scrollW: document.documentElement.scrollWidth,
    hasHScroll: layout ? layout.scrollWidth > layout.clientWidth : null,
  }
})()`)
      console.log(pc, JSON.stringify(r))
    }
    ws.close()
  } finally {
    edge.kill()
