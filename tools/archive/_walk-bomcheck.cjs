/** _walk-bomcheck.cjs — 验证 BOM 面板新字段(物料种类/物料规格/外观要求)正常渲染 */
...
async function main() {
  ...
  const profile = fs.mkdtempSync(path.join(os.tmpdir(), 'yj-bom-'))
  ...
  await send('Page.navigate', { url: FRONT + '/#/panelx/list/BOM' }); await sleep(4000)
  await evaluate(`(() => { const s = document.querySelector('.wz-skip'); if (s) s.click(); return 1 })()`); await sleep(800)
  const result = await evaluate(`(() => ({
    hasError: [...document.querySelectorAll('.el-message--error')].map(e => e.textContent.trim().slice(0, 60)),
    hasBomTable: !!document.querySelector('.bom-md'),
    masterCols: [...document.querySelectorAll('.bom-md-sec:first-child .el-table__header th')].map(t => t.textContent.trim()).filter(Boolean),
    childCols: [...document.querySelectorAll('.bom-md-sec:last-child .el-table__header th')].map(t => t.textContent.trim()).filter(Boolean),
  }))()`)
