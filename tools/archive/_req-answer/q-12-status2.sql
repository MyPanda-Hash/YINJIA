...
SELECT panel_code, COUNT(*) AS 单数,
       ...
       SUM(CASE WHEN ISNULL(canceled,0)=1 THEN 1 ELSE 0 END) AS 已弃审,
...
GO
PRINT N'=== 链路占用(暂收→检验 等) 表清单 ===';
SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME LIKE N'%link%' OR TABLE_NAME LIKE N'%occupy%';
GO
