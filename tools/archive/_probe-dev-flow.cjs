...
async function main() {
  ...
  try {
    ...
    try {
      ...
      // 2026-09-20 起按钮改名并改语义:未分发=「分发责任人」(可点)、已分发=「改责任人」(仍可点,随时换人)
      const DEV_BTN = ['分发责任人', '改责任人', '产品开发', '已下发']
      const devBtn = (btns1 || []).find((b) => DEV_BTN.includes(b.t))
      console.log(`     焦点单 = ${no1}(${f1.ok ? '已定位' : '定位失败'})`)
      console.log('     侧栏按钮:' + JSON.stringify((btns1 || []).map((b) => b.t + (b.off ? '(灰)' : ''))))
      if (devBtn && devBtn.t === '改责任人' && !devBtn.off) ok('已分发产品的按钮 = 「改责任人」且可点(分发后可随时换人)')
      else bad(`按钮应为「改责任人」可点,实际 ${JSON.stringify(devBtn)}`)
      ...
      const devBtn2 = (btns2 || []).find((b) => DEV_BTN.includes(b.t))
      console.log(`     草稿单 = ${no2}(${f2.ok ? '已定位' : '定位失败'})`)
      console.log('     草稿单侧栏按钮:' + JSON.stringify((btns2 || []).map((b) => b.t + (b.off ? '(灰)' : ''))))
      if (devBtn2 && devBtn2.t === '分发责任人' && devBtn2.off) ok('草稿单:「分发责任人」置灰(仅已归档可分发)')
      else bad(`草稿单按钮应「分发责任人」置灰,实际 ${JSON.stringify(devBtn2)}`)
