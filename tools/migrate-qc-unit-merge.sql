...
GO
-- ② 特采单:总数量 转文本并拼单位(分批执行:同批编译期 DDL 后的引用列不存在;
--   幂等守卫:总数量已是 nvarchar 文本型则整段跳过,防重复执行造成 '200支支' 双拼)
IF TYPEPROPERTY(COLUMNPROPERTY(OBJECT_ID('dbo.qc_tc_in'), N'总数量', 'xusertype'), 'precision') IS NOT NULL
   ...
   PRINT N'[qc-unit-merge] 特采总数量已是文本型,§② 跳过(幂等)';
ELSE BEGIN
IF COL_LENGTH('dbo.qc_tc_in', N'总数量文本') IS NOT NULL ALTER TABLE qc_tc_in DROP COLUMN [总数量文本];
GO
IF NOT EXISTS (SELECT 1 FROM sys.columns c WHERE c.object_id = OBJECT_ID('dbo.qc_tc_in') AND c.name = N'总数量' AND TYPE_NAME(c.system_type_id) LIKE 'nvarchar%')
EXEC sp_rename N'dbo.qc_tc_in.总数量', N'总数量文本', 'COLUMN';
GO
IF NOT EXISTS (SELECT 1 FROM sys.columns c WHERE c.object_id = OBJECT_ID('dbo.qc_tc_in') AND c.name = N'总数量')
ALTER TABLE qc_tc_in ADD [总数量] nvarchar(50) NULL;
GO
IF COL_LENGTH('dbo.qc_tc_in', N'总数量文本') IS NOT NULL
UPDATE qc_tc_in SET 总数量 = CAST(总数量文本 AS nvarchar(30))
  + CASE WHEN ISNULL(计量单位,N'')<>N'' THEN 计量单位 ELSE N'' END;
GO
IF COL_LENGTH('dbo.qc_tc_in', N'总数量文本') IS NOT NULL
ALTER TABLE qc_tc_in DROP COLUMN [总数量文本];
END
GO
