/* 验证左侧菜单:数据记录表/产品文件/实验室使用记录表 与研发管理同级 */
...
async function main() {
  ...
  try {
    ...
    const topItems = await evaluate(`[...document.querySelectorAll('.nav-group')].map(el => el.textContent.trim())`)
    console.log('top-level nav groups:', JSON.stringify(topItems))
    const hasTop = ['研发管理','数据记录表','产品文件','实验室使用记录表'].every(t => topItems.join(',').includes(t))
    // 研发管理下的二级
    const rdChild = await evaluate(`(() => { const g = [...document.querySelectorAll('.nav-group')].find(el => el.textContent.includes('研发管理')); if (!g) return []; return [...document.querySelectorAll('.nav-module')].map(el => el.textContent.trim()) })()`)
    console.log('all top present:', hasTop, '| modules sample:', JSON.stringify(rdChild.slice(0, 10)))
    ws.close()
