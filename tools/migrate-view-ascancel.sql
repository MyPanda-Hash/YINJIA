-- migrate-view-ascancel.sql — 三个报表视图补 asp_cancel 虚拟列(通用列表 COUNT 包装 ISNULL(t.asp_cancel,'N')<>'Y' 需要)
SET NOCOUNT ON;
...
WHERE l.[批号] IS NOT NULL AND ISNULL(l.asp_cancel,''N'')<>''Y'';');
GO
PRINT N'migrate-view-ascancel 完成';
GO
