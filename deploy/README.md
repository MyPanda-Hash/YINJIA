...
>   库空时=全量写入(约 30-40 分钟/6千张;档案另约 4600 条/46 页),库满时=指纹跳过(十几秒);
...
> - 两个脚本**默认都只同步已审核单据**,且**一次运行覆盖销售+采购两类订单 + 12 类基础资料档案**
>   (config.sync.types 可增减;档案条目以 `BD_` 开头,先于订单执行——商品的所属类别依赖商品分类先落表);
