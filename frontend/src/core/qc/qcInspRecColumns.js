 ...
 *  `locked`:需要**回填**的字段(物料批次)只读 —— 批次号在采购入库审核时才分配并回填
 *  (BatchService 回填检验单/检验目录/本报告),录入阶段不允许手填,故纸面上显示为文本;
 *  程序化回填(检验目录→本报告跳转、后端回填)不受影响。 */
export const QC_INSP_REC_HEAD_ROWS = Object.freeze([
  ...
  [{ label: '物料批次', key: '物料批次', locked: true }, { label: '文件编码', key: '文件编码' }],
