...
const RD_MOLD_PROC_SEC0 = [
      ...
      { page: 1, bar: '检验要求', rows: [
        ...
        { grid: [
          // 2026-09-20 用户口径:外径/内径两块**各自对半平分**,行总宽 1040 不变。
          // 设计源(R24)里是 B24=外径mm、C24=外径公差、D24=内径mm、E24:G24=内径公差 ——
          // 落到网格上会变成 110|70 与 100|210,两块都不对半;改为 B=C=90、D+E=F+G=155:
          // 外径 90|90、内径 155|155,都是正对半。列宽只在**块内**挪,块的总宽
          // (B:C=180 / D:G=310 / H:K=420)一分未动 ⇒ 其它行的格子宽度逐像素不变。
          { key: '外径mm' },
          { key: '外径公差' },
          { key: '内径mm', span: 2 },
          { key: '内径公差', span: 2 },
