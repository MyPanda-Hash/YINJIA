...
export const recordSheetConfigs = {
  ...
  RD_MOLD_PROC: {
    ...
    // 列宽(2026-09-11 重排):原 [130,110,70,100,70,70,70,100,70,125,125] 是 1028——用于配方页时
    //「炭棒规格」要拆成 炭棒规格1/2/3 三格(网格第 2-4 列必须等宽 70),「产品管控类型[4]+外观要求[2]」
    // 两格合并跨度需覆盖网格 C..G(70+100+70+70+70=380),故 C/D 两列各匀出 5/10px 给 J/K。
    // 总宽 1040(与其他文书面板一致,打印 0.92 zoom 后仍是一页)。
    grid: [130, 110, 70, 90, 70, 70, 70, 100, 70, 130, 130],
    ...
    sections: [
      ...
      // ═══ 页 2:成型配方(原 RD_MOLD_FORMULA;全部 page:1) ═══
      // 产品基本信息与页 1 同源(同一张纸同一个头,同一个 产品编号),但配方表要 13 格铺进 11 列网格,
      // 故这一页的跨度分配与页 1 不同 —— 区块的「值行」必须与「标签行」跨度严格一致(竖线才对齐):
      //   产品编号[2]=130+110 · 产品名称[2]=70+90 · 炭棒规格[3]=70+70+70
      //   产品管控类型[4]+外观要求[2]=70+100+70+70+70 ;生产车间 单独占行(网格只有 11 列,塞不下第 4 组)
      { page: 1, bar: '产品基本信息', rows: [
        ...
        { grid: [
          ...
          { key: '炭棒规格2' },
          { key: '炭棒规格3' },
          { key: '产品管控类型', type: 'select', span: 4 },
          { key: '外观要求', type: 'select', span: 2 },
        ]},
        { grid: [
          { label: '生产车间', span: 2 },
          { key: '生产车间', type: 'select', span: 9 },
        ]},
      ]},
    ],
    dataTables: [
      // 配方表(原 RD_MOLD_FORMULA 的 13 格:No.1 / 物料种类3 / 物料编号4 / 物料名称3 / 比例1 / 含量1 / 设计添加量1 = 11 列)——
      // 落同一张明细表(rd_mold_proc_detail),用「表区=配方表」分块(filterKey 模式,与规格书 dataTables 同款);
      // totalCols 合计行对 比例/含量/设计添加量 数值列求和(合计格 colspan 跟着首列跨度走)。
      { page: 1, bar: '配方表', autoSeqBar: true, totalCols: true, filterKey: '表区', filterVal: '配方表', cols: [
          { key: '序号', label: 'No.' },
          { key: '物料种类', label: '物料种类', span: 3 },
    ...
    // 配料要求(原 RD_MOLD_FORMULA 的 tailSections;须 page:1,否则会串到工艺清单页)
    tailSections: [
  ...
  // ⚠ 2026-09-11 起本面板已并入 RD_MOLD_PROC 第 2 页签、从菜单下线;此处配置保留只为回滚参考,
  //   不再渲染(RECORD_SHEET_PANELS 仍含本键,但菜单已无入口)。
  RD_MOLD_FORMULA: {
    headMode: 'report',
  ...
  // ⚠ 2026-09-11 起本面板已并入 RD_ASM_PROC 第 2 页签、从菜单下线;此处配置保留只为回滚参考,
  //   不再渲染(RECORD_SHEET_PANELS 仍含本键,但菜单已无入口)。
  RD_ASM_BOM: {
    headMode: 'report',
    staticTitle: '组装BOM表',
    ...
    dataTables: [
      { bar: '二、炭棒滤芯组装/包装物料清单', filterKey: '表区', filterVal: '物料清单',
  ...
  RD_ASM_PROC: {
    headMode: 'plain',
    // plain 版式标题条:多页签面板给**数组**(按页签取,缺省回退第 0 个)——
    // 字符串仍是单页老面板的写法(实验室记录表等),两种都支持。
    plainTitle: ['炭棒滤芯组装/包装段-关键工序控制清单', '组装BOM表'],
    pages: [
      { title: '组装工艺清单' },
      { title: '组装BOM表' },
    ],
    // 产品基本信息分区:页 1 顶部(plain 版式没有报告头,这一区就是抬头)+ 页 2(组装BOM表产品信息)。
    // 「产品编号」是 参照 RD_PROD_INFO.产品编号 → 带回 产品名称 的关联字段(与成型侧同口径),
    // DevTaskService.PRODUCT_KEY / 产品开发状态角标 / 产品开发下发都靠它。
    sections: [
      { bar: '产品基本信息', rows: [
        { label: '产品编号', key: '产品编号', type: 'text' },
        { label: '产品名称', key: '产品名称', type: 'text' },
      ]},
      // ═══ 页 2:组装BOM表 —— 产品基本信息补齐(原 RD_ASM_BOM「一、产品基本信息」余下 4 格)═══
    ...
    dataTables: [
      // ═══ 页 1:关键工序控制清单(21 道工序预置;w 合计 1040 = 纸张可印宽)═══
      { seedRows: [
          ...
          { key: '工序', label: '工序', w: 150 },
          { key: '工序控制内容', label: '工序控制内容', w: 300, area: true },
          { key: '管控要求', label: '管控要求', w: 390, area: true },
          { key: '检查比例', label: '检查比例', w: 200 },
        ]},
      // ═══ 页 2:物料清单(原 RD_ASM_BOM「二、炭棒滤芯组装/包装物料清单」;行落在 rd_asm_proc_detail,表区=物料清单)═══
      // plain 版式下数据表自持列宽(cols.w);「物料规格/外观要求」原为两行高文本域,与「物料名」同排时按单行渲染,
      // 内容过长会在行内换行(不再撑高整行),需要多行的场景仍可切回 RD_ASM_BOM 配置口径。
      { page: 1, bar: '二、炭棒滤芯组装/包装物料清单', filterKey: '表区', filterVal: '物料清单',
        ...
        cols: [
          ...
          { key: '物料名', label: '物料名', w: 140 },
          { key: '物料编号', label: '物料编号', w: 130 },
          { key: '物料规格', label: '物料规格', w: 280 },
          { key: '外观要求', label: '外观要求', w: 280 },
          { key: '用量', label: '用量', w: 210 },
        ]},
      // ═══ 页 2:修订记录(原 RD_ASM_BOM「修订记录」;表区=修订记录)═══
      { page: 1, bar: '修订记录', filterKey: '表区', filterVal: '修订记录', cols: [
          { key: '表区', label: '表区', hiddenCol: true },
          { key: '序号', label: '序号', w: 70 },
          { key: '更改内容', label: '更改内容', w: 300, area: true },
          { key: '更改原因', label: '更改原因', w: 200 },
          { key: '更改时间', label: '更改时间', w: 130 },
          { key: '责任人', label: '责任人', w: 120 },
          { key: '备注', label: '备注', w: 220 },
        ]},
    ],
  },
  ...
  RD_SPEC_DOC: {
    ...
    sections: [
      // 第 1 页(产品信息):封面之下接 1.适用范围 / 2.整体规格参数 / 3.产品主要性能——
      // 《规格书细分.xlsx》「页面-产品信息」含两张设计图(封面 495×724 + 本节章节块 509×87),
      // 这三节本就属产品信息页;原先误挂第 3 页,2026-09-11 归位。
      // **序号保持受控文件原编号**:1/2/3 随页走,第 3 页仍从 4.产品性能检验项目及检验标准 开始,不重排。
      { page: 0, doc: true, rows: [
